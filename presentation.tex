...
\begin{frame}{Gradient Boosting Model — Benek Kurtyka}
  \begin{columns}[T]
    ...
    \textbf{Approach}
    \begin{itemize}
      \item XGBoost / LightGBM classifier
      \item Same 24 trade-only features as the DT
      \item Hyperparameter tuning on validation split
    \end{itemize}
...
\begin{frame}{Sequence Model (LSTM) — Krzysztof Nalazek}
  \begin{columns}[T]
    ...
    \textbf{Approach}
    \begin{itemize}
      \item Causal LSTM over a window of recent trades
      \item Window size: 32 trades
      \item Input: 16 trade-only features (no absolute-price features)
      \item Trained on both symbols (pooled)
    \end{itemize}
...
\begin{frame}{Full Model Comparison — Test Set (Apr 14)}
  ...
  \begin{block}{Baseline to beat}
    All three team models target Quote Rule / Lee-Ready as the primary benchmark
    ($\text{accuracy} = 91.64\%$, $\text{errors} = 50{,}853$).
  \end{block}
\end{frame}
